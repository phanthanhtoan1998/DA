...
# df['District'] = df['District'].str.replace('Quận','').str.replace('Địa chỉ','').str.replace(':','').str.replace(' ','').str.strip()
# df['Ward'] = df['Ward'].str.replace('Phường','').str.replace('Địa chỉ','').str.replace(':','').str.replace(' ','').str.strip()
# df['Area'] = df['Area'].str.replace('m2','').str.strip().astype(float)
# df = df[df['Price'] != 'Thỏa thuận']

# df.loc[df['Price'].str.contains('triệu/tháng', na=False), 'Price'] = (df.loc[df['Price'].str.contains('triệu/tháng', na=False), 'Price'].str.replace(' ','').str.replace('triệu/tháng','').str.replace(',','.').astype(float)*1000000).astype(int)
# df.loc[df['Price'].str.contains('trăm/tháng', na=False), 'Price'] = (df.loc[df['Price'].str.contains('trăm/tháng', na=False), 'Price'].str.replace(' ','').str.replace('trăm/tháng','').str.replace(',','.').astype(float)*100000).astype(int)
# df.loc[df['Price'].str.contains('đồng/tháng', na=False), 'Price'] = (df.loc[df['Price'].str.contains('đồng/tháng', na=False), 'Price'].str.replace(' ','').str.replace('đồng/tháng','').str.replace(',','.').astype(float)).astype(int)
...
# df_cleaned = df_cleaned.drop(['District', 'Ward', 'Month', 'Year'], axis=1)
...
regr = RandomForestRegressor(max_depth=83, random_state=0)
